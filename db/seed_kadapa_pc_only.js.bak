// Seeds the SQLite demo db with REAL Badvel (SC) Assembly Constituency data,
// sourced from ECI results as compiled on Wikipedia (Badvel_Assembly_constituency),
// cross-checked against Oneindia's 2024 results page.
//
// Scope: AC-level and mandal-list only. Booth-level and mandal-wise vote
// splits are NOT publicly available for Badvel in the sources checked, so
// they are intentionally left unseeded rather than fabricated — see README.
...
const pcId = lastId(run(
  `INSERT INTO admin_units (parent_id, level, name, code) VALUES (?, 'PC', 'Kadapa', 'S4-25')`,
  [stateId]
));
const acId = lastId(run(
  `INSERT INTO admin_units (parent_id, level, name, code, reservation, lok_sabha_pc_id)
   VALUES (?, 'AC', 'Badvel', '124', 'SC', ?)`,
  [stateId, pcId]
));

const mandals = ['Kalasapadu', 'B.Kodur', 'Sri Avadhutha Kasinaya', 'Porumamilla', 'Badvel', 'Gopavaram', 'Atlur'];
const mandalIds = {};
for (const m of mandals) {
  mandalIds[m] = lastId(run(
    `INSERT INTO admin_units (parent_id, level, name) VALUES (?, 'MANDAL', ?)`,
    [acId, m]
  ));
}

// Second AC — Kadapa (general seat, urban, same PC) — proves the hierarchy
// and API generalize beyond one seat. Contrast with Badvel: unreserved,
// single urban mandal vs 7 rural mandals, and TDP broke YSRCP's streak
// here in 2024 while Badvel stayed with YSRCP.
const acId2 = lastId(run(
  `INSERT INTO admin_units (parent_id, level, name, code, reservation, lok_sabha_pc_id)
   VALUES (?, 'AC', 'Kadapa', '125', NULL, ?)`,
  [stateId, pcId]
));
const kadapaMandalId = lastId(run(
  `INSERT INTO admin_units (parent_id, level, name) VALUES (?, 'MANDAL', 'Kadapa')`,
  [acId2]
));
...
for (const [abbr, color] of Object.entries(partyColors)) {
  partyIds[abbr] = lastId(run(
    `INSERT INTO parties (name, abbr, color_hex) VALUES (?, ?, ?)`,
    [abbr, abbr, color]
  ));
}

// ---- elections + results ----
// Each entry: year, is_by, [ {name, party, votes}, ... ] winner is highest votes.
// electors/votes_polled/turnout_pct sourced alongside results where available.
const elections = [
  {
  ...
  },
  {
    ...
    candidates: [
      ...
      { name: 'Singamala Venkateswarlu', party: 'IND', votes: 9574 }, // PRP, mapped to IND bucket
...
// Kadapa AC — real results 2004(as Cuddapah)/2009/2014/2019/2024, sourced
// from Wikipedia's Kadapa Assembly constituency page (ECI-derived).
// No by-election on record for this seat, unlike Badvel.
const kadapaElections = [
  ...
  },
  {
    ...
    candidates: [
      ...
      { name: 'Afzal Alikhan T.K.', party: 'IND', votes: 13989 } // PRP, bucketed as IND
    ]
  },
  {
...
function needsParty(abbr) {
  ...
  partyIds[abbr] = lastId(run(
    `INSERT INTO parties (name, abbr, color_hex) VALUES (?, ?, ?)`,
    [abbr, abbr, partyColors[abbr] || '#888780']
  ));
}
needsParty('JSP');
...
function seedElections(targetAcId, electionsData) {
  for (const el of electionsData) {
    run(
      `INSERT OR IGNORE INTO elections (election_type, year, is_by_election) VALUES ('AC', ?, ?)`,
      [el.year, el.isBy]
    );
    const electionId = one(
    ...
    run(
      `INSERT INTO turnout (admin_unit_id, election_id, electors, votes_polled, turnout_pct)
       VALUES (?, ?, ?, ?, ?)`,
      [targetAcId, electionId, el.electors, el.votesPolled, el.turnoutPct]
    );

    // Classification rule (simple, transparent, swappable later):
    // margin_pct = (winner - runner-up) / total_votes * 100
    //   >= 15  -> STRONGHOLD for winning party
    //   5-15   -> SWING
    //   < 5    -> WEAK (contested razor-thin)
    ...
    run(
      `INSERT INTO area_classification (admin_unit_id, election_id, party_id, classification, margin_pct)
       VALUES (?, ?, ?, ?, ?)`,
...
seedElections(acId, elections);
seedElections(acId2, kadapaElections);

// ---- demographics (Census 2011/2019 elector counts, area-level only, no voter linkage) ----
run(
  `INSERT INTO demographics (admin_unit_id, census_year, population, rural_pct, sc_pct, st_pct)
   VALUES (?, 2011, 274179, 74.24, 21.73, 1.8)`,
  [acId]
);
// Kadapa AC is the district HQ town seat — predominantly urban, unlike Badvel.
// No separate Census village-directory figure sourced yet for this AC;
// left unseeded rather than guessed (see README).

console.log('Seeded election_data.sqlite with real data for 2 ACs under Kadapa PC:');
console.log(`  admin_units: state=${stateId}, pc(Kadapa)=${pcId}`);
console.log(`  Badvel AC=${acId} (${mandals.length} mandals), elections: ${elections.map(e => e.year + (e.isBy ? 'by' : '')).join(', ')}`);
console.log(`  Kadapa AC=${acId2} (1 mandal), elections: ${kadapaElections.map(e => e.year).join(', ')}`);
db.close();
