...
// Default landing point when no AC was pre-selected — Badvel has full real
// multi-election data, so it's the most informative first view.
const DEFAULT_DISTRICT = 'YSR Kadapa';
const DEFAULT_AC = 'Badvel';

export default function ConstituencyView({ onBack, initialAc }) {
  const [districts, setDistricts] = useState([]);
  const [selectedDistrict, setSelectedDistrict] = useState(null);
  const [acs, setAcs] = useState([]);
  const [selectedAc, setSelectedAc] = useState(initialAc || null);
  ...
  // Load all 26 districts once, on mount. If an AC was pre-selected (came
  // from the District view), default to its district; otherwise YSR Kadapa.
  useEffect(() => {
    (async () => {
      try {
        ...
        let defaultDistrict = districtList.find(d => d.name === DEFAULT_DISTRICT) || districtList[0];
        if (initialAc) {
          const resolved = await api.resolve(initialAc.name, 'AC');
          const match = resolved[0];
          if (match) {
            const parentDistrict = districtList.find(d => d.id === match.parent_id);
            if (parentDistrict) defaultDistrict = parentDistrict;
          }
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setAcs(acList);
        const preselect = initialAc && acList.find(a => a.id === initialAc.id);
        const defaultAc = preselect || acList.find(a => a.name === DEFAULT_AC) || acList[0];
